...
// 2 * numberLength がほぼ定数に近い働きをしており、これを変数にすることが難しい。よって'A'~'Z'は2文字の扱いをした。 //
{
    ...
    unsigned int contentLength = (unsigned int)[japanpostContents length];
    ...
    if ([[self initiator] length] > index) {
		...
		if (index >= (contentLength + 1) * (2 * numberLength) + [[self initiator] length]) {
			bar = (index - (unsigned int)[[self initiator] length] - (contentLength + 1) * (2 * numberLength)) % [[self terminator] length];
			hexDigit = closeBracket;
			descriptor = japanpost_barDescriptor(hexDigit,bar,closeBracketLength);
		}
		else {
			unsigned int digit = (index - (unsigned int)[[self initiator] length]) / (2 * numberLength); // 何文字目か //
...
// 大文字化する。20文字に揃える。使用できない文字を排除する。郵便番号の整合性はチェックしない。 //
{
	...
	if (maxLength > [tempStr length]) {
		for (i = (unsigned int)[tempStr length]; i < maxLength; i++)
			[tempStr appendString:@"="]; // CC4 //
	}
	else {
		if (maxLength < [tempStr length]) {
			unsigned int validCount;
			unsigned int tempL = (unsigned int)[tempStr length];
			...
			tempL = (unsigned int)tempStr.length;
			validCount = 0;
